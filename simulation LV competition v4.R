...
for (t in 1:duree){
  ...
  if (n_i[pheno_maj] > 0.95*(n_i[pheno_maj]/sum(n_i))){
    if (runif(1)>0.5){
      n_i[pheno_maj+1] = n_i[pheno_maj+1] + 0.10 * n_i[pheno_maj]
      n_i[pheno_maj] = 0.9 * n_i[pheno_maj]
      
    }
    else{
      n[pheno_maj-1] = n[pheno_maj+1] + 0.10 * n[pheno_maj]
      n_i[pheno_maj] = 0.9 * n_i[pheno_maj]
    }
    
    n_i[n_i[pheno_maj] < 0.4*(n_i[pheno_maj]/sum(n_i))] = 0
  }
